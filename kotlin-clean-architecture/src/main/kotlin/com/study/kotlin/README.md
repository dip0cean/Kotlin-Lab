...
- 경계 간 매핑 별 장단에 대해 살펴보자
    - 매핑을 할 경우 : 양 계층에서 필요한 모델을 각각 사용하기 때문에 계층 간 결합도가 약해진다.
    - 매핑을 하지 않을 경우 : 계층 별로 필요한 모델을 관리해야 하니 이로 인해 불필요한 보일러 플레이트 코드가 많아지는 문제가 있다.

#### 매핑하지 않기 전략

> 모든 계층에서 Domain 클래스를 공용으로 사용하는 전략이다. 
> 매핑을 하지 않는 전략을 사용할 경우 각 계층에서 관리해야 하는 모델의 수가 줄어든다는 장점이 있다.
> 
> 웹 계층이나 영속성 계층에서 특별한 요구사항이 추가되는 경우 모든 요구사항을 반영하기 위해서 Domain 클래스를 수정해야 하는데, 이 때 Domain 클래스는 **단일 책임 원칙** 이 깨지게 되는 단점을 가진다.
> 
> 즉, 여러 계층에서 사용하면 공통으로 사용되는 부분을 Domain 클래스에서 한 번에 처리하니 보일러 플레이트 코드가 줄어든다는 장점이 있지만, 요구사항이 추가되거나 변경 사항이 적용 되어야 하는 경우 단일 책임 원칙이 깨져 오히려 유지보수가 어려워질 수 있다는 단점이 있다.
> 
> 만약 모든 계층에서 정확히 동일한 구조를 가지고, 주고 받는 데이터의 정보가 완벽히 동일한 경우 이 '매핑하지 않기 전략'이 좋은 선택지가 될 것이다.

#### 양방향 매핑 전략

> 웹 / 애플리케이션 / 영속성 계층 별로 요구되는 모델을 각각 관리하도록 하는 방식이다. 
> 
> 이러한 전략을 취할 경우 각 계층의 전용 모델만 사용하기 때문에 요구사항 변경으로 인해 특정 계층의 모델이 변경되더라도 다른 계층에는 영향을 주지 않는다.
> 
> 또한, 웹이나 영속성 관심사로 오염되지 않는 **깨끗한 도메인 모델** 을 구현할 수 있어 JSON 이나 ORM 매핑 애노테이션이 필요하지 않게 된다. 
> 즉, **단일 책임 원칙** 을 지킬 수 있게 되는 것이다.
> 
> 하지만 해당 전략은 너무 많은 보일러 플레이트 코드를 생성하고 관리해야 한다는 단점이 있으며, 도메인 모델이 애플리케이션 계층을 넘어 웹 계층까지 영향을 미치게 되어 웹 계층에서 발생하는 요구사항 변경에 약해진다는 취약점이 있다.
